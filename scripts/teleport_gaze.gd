...
func _process(delta: float) -> void:
    var cam = get_node("../Camera3D") if has_node("../Camera3D") else get_tree().get_root().get_node_or_null("root/Camera3D")
    if not cam:
    ...
    var res = space.intersect_ray(from, to, [self, cam])
    if res:
...
func _teleport_to(pos: Vector3) -> void:
    ...
    var cam = get_node("../Camera3D") if has_node("../Camera3D") else get_viewport().get_camera_3d()
    if not cam:
        return
